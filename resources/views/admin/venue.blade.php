...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">{{ __('Venues') }}<a href="/admin/venue/add"><button type="button"
                              class="ml-2 btn btn-primary">Add
                                New</button></a></div>
                    <div class="card-body">
                        @if (Route::is('viewvenue'))
                            @if (count($venues) > 0)
                                <table class="table">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($venues as $data)
                                            <tr>
                                                ...
                                                <td>{{ $data->status }}</td>
                                                <td>
                                                    <a href="{{ url('/admin/venue/delete', ['id' => $data->id]) }}">
                            ...
                            @endif
                        @endif
                        @if (Route::is('addvenue'))
                            <form method="POST" action="{{ route('savevenue') }}">
                                @csrf
                                <div class="row mb-3">
                                    <label for="heading"
